...
- `POST /api/boards` — print a fresh board. Validates term count (`size² - 1`, one tile is FREE at `freeIndex(size)`) and term uniqueness via the shared schema; `400 invalid_board` with a message on failure.
