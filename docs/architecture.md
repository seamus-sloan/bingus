...
The server is the referee. Cards are dealt server-side per player — shuffle the board's word bank, take the first `size² - 1` terms (so cards can't be re-rolled and rivals' boards render consistently) — and win claims are validated server-side against the marked set (row / column / diagonal / blackout). Clients render state and report intents; they never decide outcomes. A bank sized exactly to the card is the degenerate case, dealing every seat the same terms in a different order; a deeper bank lets cards differ in content too. Seats are dealt independently, so distinct cards are a matter of odds rather than a guarantee — two seats collide with probability `1 / C(bank, tiles)`, which is 1-in-9 for a 9-term 3×3 bank and vanishing for anything deeper. Forcing distinctness would couple seats together and breaks down once a table outgrows the available sets, so the deal stays independent.
...
- **Host** ([docker-compose.yml](../docker-compose.yml)) — pulls `sesloan/bingus:latest`, publishes port 3000, bind-mounts `./data:/data`. The service sets `pull_policy: always`, so `docker compose up -d` re-checks Docker Hub and is all it takes to move onto a new release. Nothing pulls on its own: `restart: unless-stopped` replays the cached image, and a moved `latest` tag never reaches a container that is already running — publishing a release does not deploy it.
